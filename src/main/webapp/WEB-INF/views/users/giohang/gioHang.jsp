                ...
                <tbody>
                    <c:forEach var="model" items="${gioHang}">
                        <tr class="cart-item" id="form-cart">
                            ...
                            <td class="product-price"><span class="gia">${model.getProductDTO().getGia() }</span></td>
    ...
    if (table.rows.length <= 1) {
 	...
 	function deleteOne(tenProduct) {
 		var confirmation = confirm("Bạn muốn xóa sản phẩm này khỏi giỏ hàng không:<<");
	 	 if (confirmation) {
		$.ajax({
			url:'/suabot/gioHang/deleteSP/'+tenProduct,
			type: 'POST',
			dataType: 'json'
		})
		.then(data =>{
...
<script type="text/javascript">
    ...
    function thanhToan() {
        ...
        if(totalamount === ''){
            ...
            window.location.href="/suabot/thanhtoan/"+totalamount+"/<%=SecurityUntil.getPrincipal().getUsername() %>" 
        }
