...
@RestController(value = "gioHangAPIUser")
public class GioHangUserAPI {
	@Autowired
	private IProductService productService;
	private String gioHang1 = "cart";
	
	@PostMapping(value = "/gioHang/{id}")
	public CartDTO addGioHang(HttpSession session,@RequestBody ProductDTO productDTO, @PathVariable long id) {
	...
	@PostMapping(value = "/gioHang/deleteSP/{tenSanPham}")
	public CartDTO deletePage(HttpSession session, @PathVariable String tenSanPham) {
	    List<CartDTO> models = (List<CartDTO>) session.getAttribute(gioHang1);
	    CartDTO mess = new CartDTO();
